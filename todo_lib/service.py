...
def add_todo(
    ...
    Raises:
        ValidationError: 제목이 비어 있거나 날짜/우선순위 형식이 잘못된 경우.
    """
    ...
    item = create_todo(session, title=title, due_date=due_date, priority=priority)
...
def list_todos(
    ...
    if priority is not None:
        priority = validate_priority(priority)
    return get_all_todos(session, is_done=filter_done, priority=priority)
